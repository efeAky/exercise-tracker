...
import addRoutineAction from "@/serverActions/routines/addRoutine";

interface AddRoutineFormProps {
  userId: number;
}

export default function AddRoutineForm({ userId }: AddRoutineFormProps) {
  const [formData, setFormData] = useState({
    routineName: "",
    exercises: [{ exerciseName: "", goaledSets: "", goaledReps: "" }],
  });
  ...
  const addRoutineHandle = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setError("");
    setIsLoading(true);
    ...
    try {
      await addRoutineAction(userId, routineNameTrimmed, exercisesValidated);
  ...
  return (
    <div className="w-full max-w-2xl">
      <div className="flex flex-col gap-6">
        ...
        <form onSubmit={addRoutineHandle} className="flex flex-col gap-6">
          <div>
