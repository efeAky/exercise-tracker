...
import enterProgress from "@/serverActions/workouts/enterProgress";

interface EnterProgressProps {
  routine: Routine;
}

export default function EnterProgressForm({ routine }: EnterProgressProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    exercises: routine.exercises.map((ex) => ({
      ...
      actualsets: Array.from({ length: ex.goaledsets ?? 3 }, () => ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    for (let i = 0; i < formData.exercises.length; i++) {
      ...
      for (let j = 0; j < ex.actualsets.length; j++) {
        if (ex.actualsets[j].reps === undefined) {
          setError(
            `Exercise "${ex.exercisename}" Set ${j + 1}: please enter reps`
          );
          setIsLoading(false);
          return;
        }
                  ...
                  {exercise.actualsets.map((set, setIndex) => (
                    <div key={setIndex}>
                      ...
                      <input
                        ...
                        value={set.reps ?? ""}
                        onChange={(e) =>
                          handleSetChange(exerciseIndex, setIndex, e.target.value)
                        }
